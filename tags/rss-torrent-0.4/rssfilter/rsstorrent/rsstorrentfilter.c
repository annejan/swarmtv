...
static int disectdescription(unsigned char *desc, unsigned char** category, int *seeds, int *peers, size_t *size) {
  ...
  if (p == NULL) {
    ...
    fprintf(stderr, "Ouch! Can't compile regular expression: %s (char %i)", errmsg, errpos);
  }
  ...
  if (rc < 0) {
    switch (rc) {
      case PCRE_ERROR_NOMATCH:
        printf("String could not be split.\n");
        printf("%s\n", desc);
        break;
...
static int disectdate(char *date, time_t *pubdate)
{
  ...
  if(rc == NULL) {
    fprintf(stderr, "Failed to convert time '%s'\n", date);
    return -1;
  }
...
int rsstorrentfilter(sqlite3 *db, char *name, char *url, char *filter, MemoryStruct *rssfile)
{
  ...
  for(count = 0; count < nodeset->nodeNr; count++) {
    ...
    if( rc < 0 ) {
      fprintf(stderr, "No pubdatestr found\n");
    }
    //printf("pubdatestr: %s\n", pubdatestr);
    ...
    if(rc != 0) {
       ...
       */
      fprintf(stderr, "description failed: %s\n", description);
      continue;
    }
