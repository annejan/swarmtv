 ...
 *	Tweat instead of download by Anne Jan Brouwer 2010 
 */
...
#include "torrenttwitter.h"

/*
 * Program version
 */
#define PROGVERSION "0.4-twitter"
...
void printhelp(void) 
{
  printf( "################\n"
          ...
          "-F <name:url>    : Set tweet filter (empty default duplicate filter).\n"
...
int main(int argc, char **argv){
  ...
  writelog(LOG_NORMAL, "Start rss-torrent-twitter");
  ...
  while( opt != -1 && stopop == 0) {
    switch( opt ) {
      case 'v':
        printf("RSSTorrent-twitter by Paul Honig and Anne Jan Brouwer 2009-2010\n");
        printf("Version %s \n", PROGVERSION);
        break;
      case 'c':
        printconfigitems(db);
